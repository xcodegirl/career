"""
Generate PDF resume from JSON source using LaTeX.
Calls json2tex.py to generate LaTeX, then compiles with pdflatex.
Usage: python json2pdf.py input.json [output.pdf]
"""
import json
import os
import sys
...
def generate_latex(json_path, tex_path, scripts_dir):
    """Call json2tex.py to generate LaTeX from JSON."""
    try:
        result = subprocess.run(
            [sys.executable, os.path.join(scripts_dir, 'json2tex.py'), json_path, tex_path],
            capture_output=True,
            text=True,
            timeout=30
        )
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python json2pdf.py input.json [output.pdf]")
        sys.exit(1)
    ...
    try:
        ...
        print(f"\nSuccessfully generated CV: {output_pdf}")
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
