\documentclass[10pt]{article}
\usepackage[letterpaper, margin=0.75in]{geometry}
\usepackage{enumitem}
\usepackage[hidelinks]{hyperref}
\usepackage{titlesec}

\setlength{\parindent}{0pt}
\setlength{\parskip}{0pt}
\titleformat{\section}{\large\bfseries}{}{0em}{}[\titlerule]
\titlespacing{\section}{0pt}{8pt}{4pt}
\setlist[itemize]{leftmargin=*, nosep, topsep=2pt}
\renewcommand{\labelitemi}{\textbullet}

\begin{document}
\pagestyle{empty}

\begin{center}
{\LARGE\bfseries Joanne Hoar}\\[4pt]
{\large Technology Educator, Software Developer, Game Developer}\\[4pt]
\href{https://linkedin.com/in/joanne-hoar/}{linkedin.com/in/joanne-hoar} $\cdot$ \href{https://github.com/xcodegirl}{xcodegirl}\\
\end{center}
\vspace{4pt}

\section*{Summary}
Canadian Software Developer with 20 years of experience in video game programming, scientific modelling and cross-platform development. Expertise in multiple programming languages, game engines and SDKs. Proven track record in leading teams, mentoring developers, impressing customers and building corporate culture. Strong background in C++, Unreal Engine, iOS and Android using agile methodologies and CI/CD. Passion for algorithmic correctness, mathematics, artificial intelligence and nature.

\section*{Experience}
\textbf{Technology Instructor}, \textit{Robertson College (contract)} \hfill Jan 2025 -- present
\\\textit{\small C\#, .NET, ASP .NET Core, Java, JavaScript, React, Git, GitHub, Azure, SQL}
\\[-2pt]
Instructed 20+ course offerings across Full Stack Development and Data Analytics diploma programs. Delivered live coding sessions, maintained open-source course materials on GitHub, and achieved an average student satisfaction rating of 4.5/5.0.


\textbf{Senior Software Developer}, \textit{bitHeads Inc.} \hfill 2022 -- 2025
\\\textit{\small Unreal Engine, C++, Objective-C, Swift, iOS, Android Native, Java, Kotlin, PlayStation, Xbox, Nintendo Switch, Node.js, Groovy, C\#, Shell Script}
\\[-2pt]
Developed and maintained cross-platform client SDKs for a backend-as-a-service platform used by 50+ published games across 12 platforms including PlayStation, Xbox, Nintendo Switch, iOS, and Android. Engineered CI/CD and TDD pipelines and managed customer relationships with AAA and Indie studios.


\textbf{Co-founder/ Lead Developer}, \textit{Silicon Hanna Inc. (self-employed)} \hfill 2011 -- 2022
\\\textit{\small Unreal Engine, Objective-C / Swift, iOS, Android Native, Kotlin, Augmented Reality, MacOS, Windows}
\\[-2pt]
Designed, coded and published eleven mobile games for iOS and Android, from concept and game design through to commercial app-store release. Experimented with cutting-edge device technologies including ARKit and other emerging SDKs.


\textbf{Tools Programmer}, \textit{Fekete Associates Inc. (S\&P Global)} \hfill 2008 -- 2011
\\\textit{\small C++, MFC, Windows, Perforce, Incredibuild}
\\[-2pt]
Developed tools that used mathematical modelling to maximize production and improve efficiency for major oil \& gas corporations. Led research on a new system that reduced build times by 40\%.


\textbf{Intermediate Software Engineer}, \textit{BJ Pipeline Inspection Services Inc. (Baker Hughes)} \hfill 2005 -- 2008
\\\textit{\small C++, C, OpenGL, PHP, SQL, Neural Networks, Borland, Windows, Linux}
\\[-2pt]
Visualised in-line device data with OpenGL graphics. Implemented neural network algorithms for anomaly detection. Led development of custom web-based GIS reporting software. Improved low-level programming algorithms for data processing.


\textbf{Co-founder / App Developer / Instructor}, \textit{SuRJE Software Solutions Inc. (self-employed, contract)} \hfill 2005 -- 2008
\\\textit{\small Objective-C, C++, OpenGL, Chipmunk, PHP, JavaScript, Firebase, Google Maps, iOS, Linux}
\\[-2pt]
Created and published physics-based games for iOS. Handled technical writing, web development, Google Maps and Firebase integration and server administration. Developed course curriculum, instructed C++ programming and assessed student work with the Department of Continuing Education.


\textbf{Software Researcher / Lead Developer}, \textit{PlayStarMusic Corp.} \hfill 2005 -- 2006
\\\textit{\small C++, PHP, Javascript, .NET, Windows, Linux}
\\[-2pt]
Led development and system administration for an iTunes-like music store. Automated DRM encoding and video streaming. Developed a prototype for a Google-competitive search engine.


\textbf{Junior Software Engineer}, \textit{Codefast.com} \hfill 2004 -- 2005
\\\textit{\small C++, Qt, Wx, Shell script, Windows, Unix}
\\[-2pt]
Ported applications to operate under a new GUI library. Developed user interfaces for software lifecycle automation.


\textbf{Web Developer / VR Artist}, \textit{Nickle Arts Museum, University of Calgary (contract)} \hfill 2002 -- 2004
\\\textit{\small JavaScript, C++, OpenGL, PHP, HTML, CSS, 3D Scanning, VR}
\\[-2pt]
Designed and developed an interactive online catalogue for the Gerald Hushlak art exhibit with JavaScript-based features. Contributed original generative art from graduate research. Collaborated on a pioneering VR installation using 3D scanning and CAVE immersion technology.


\textbf{Research Assistant}, \textit{VLab Computer Graphics Group / Artificial Intelligence Research Hub} \hfill 2001 -- 2004
\\\textit{\small C++, OpenGL, PHP, MySQL, Qt, Solaris, Unix, Linux}
\\[-2pt]
Developed graphics software tools using fractal mathematics and biological modelling. Managed network setup and distributed computing infrastructure. Published five research papers and served as teaching assistant across three courses.


\section*{Education}
\textbf{Master's Degree, Computer Science}, University of Calgary \hfill 2002 -- 2004
\\\textit{\small Thesis: Agent-based Development of Natural Transportation Networks}
\\[2pt]
\textbf{Bachelor's Degree, Computer Science}, University of Calgary \hfill 1998 -- 2001
\\[2pt]

\section*{Honours \& Awards}
\begin{itemize}[nosep, leftmargin=0pt, label={}]
    \item Departmental Nomination: Governor General's Gold Medal, Faculty of Graduate Studies \hfill 2005
    \item Postgraduate Scholarship NSERC D3, Natural Sciences and Engineering Research Council of Canada \hfill 2005
    \item Graduate Student Scholarship, Alberta Learning \hfill 2004
    \item Best Student Paper Award (Co-author), ACAL \hfill 2003
    \item CPSC Departmental Research Award, University of Calgary \hfill 2003
\end{itemize}

\section*{Portfolio}
\href{https://github.com/xcodegirl/career/blob/main/xcodegirl-work-samples.md}{xcodegirl/career}\\

\section*{Published Games}
\begin{itemize}[nosep, leftmargin=*]
    \item \textbf{Ragin' Bull} -- iOS, Android (2020)
    \item \textbf{AR Picture Portal} -- iOS (2018)
    \item \textbf{Fragmented Reality} -- iOS (2018)
    \item \textbf{IceCream AR} -- iOS (2018)
    \item \textbf{Jewel Slide} -- iOS, Android (2017, 2019)
    \item \textbf{Word Slide \& Find} -- iOS (2017)
    \item \textbf{A Word Theme} -- iOS, Android (2017, 2019)
    \item \textbf{CryptoQuip} -- iOS, Android (2011, 2019)
    \item \textbf{Rafterâ€™s Loot} -- iOS (2011)
    \item \textbf{Star Chase} -- iOS (2008)
    \item \textbf{Bitsy} -- iOS (2008)
\end{itemize}

\end{document}